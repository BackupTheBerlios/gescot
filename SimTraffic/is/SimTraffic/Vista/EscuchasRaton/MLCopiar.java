...
/**
 * Clase que extiende a la escucha de ratón. Se encarga de escuchar a la acción
 * copiar, que copia una selección de elementos al portapapeles
 * 
...
public class MLCopiar extends EscuchaRaton {
	...
	/**
	 * Constructora de Copiar. Se recorren todos los tramos de la seleccion.
	 * Para cada tramo, se miran sus nodos inicial y final. Si estaban en el
	 * portapapeles, se añade el tramo a los tramos de los nodos. Si no, se
	 * añaden al portapapeles. Después se busca el nodo que tenga la posición
	 * mínima para que sirva de referencia a la hora de pegar el contenido del
	 * portapapeles.
	 * 
	 * @param modelo
	 * @param controlador
	...
	public MLCopiar(IModelo modelo, IControlador controlador, PanelMapa panel) {
		...
		if (modelo.getMapa().getSeleccion() != null) {
			for (int j = 0; j < modelo.getMapa().getSeleccion()
				...
				Nodo nodoInicial = new Nodo(tramoMapa.getNodoInicial().getPos()
						.clone());
				Nodo nodoFinal = new Nodo(tramoMapa.getNodoFinal().getPos()
						.clone());
				...
				Nodo nodoInicialEnPortapapeles = modelo.getMapa()
						.getPortapapeles().existeNodo(nodoInicial);
				...
				Nodo nodoFinalEnPortapapeles = modelo.getMapa()
						.getPortapapeles().existeNodo(nodoFinal);
